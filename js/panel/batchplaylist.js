...
const { getIconSVG } = require("../Token.js");
...
async function bpList() {
    ...
    async function addElementToDropdown(templates) {
        ...
        const _allicons = await allIcons(templates);
        await templates.forEach((tmplt) => {
            const element = document.createElement("div");
            element.className = "btn-main-panel";
            element.classList.add("btn-batchplay")
            element.setAttribute("value", tmplt.name);
            element.innerHTML = _allicons.find(x => x.filename == tmplt.name).svg;
            ...
            element.addEventListener("click", async() => {
                ...
                await bplist_token
                    .getEntry(element.getAttribute("value"))
                    .then(async(e) => {
        ...
        return new Promise(async(resolve, reject) => {
            ...
            for (let data of templates) {
                await bplist_token.getEntry(data.name).then(async(e) => {
                    const cmd = JSON.parse(await e.read());
                    if (cmd.icon) {
                        icons.push({ filename: data.name, icon: cmd.icon });
                    }
                });
            }

            resolve(await getIconSVG(icons));
    ...
    async function refreshLists(reset) {
        ...
        const templates = (await bplist_token.getEntries()).filter(
            (tmplt) => tmplt.name.indexOf("json") > 0
        );
